...
func sendApduWithoutResponse(c spi.Conn, apdu []byte) ([]byte, error) {
	...
	// next command without reset CRC
	// // TXCrcPreset
	// if err := write(c, 0x2C, []byte{0x19}); err != nil {
	// 	return nil, err
	// }
	...
	defer func() { fmt.Printf("time transceive: %v\n", time.Since(tt)) }()
	// IRQ1 register
	if err := waitTxIRQ(c, 0x02, 20*time.Millisecond); err != nil {
		return nil, err
	}
