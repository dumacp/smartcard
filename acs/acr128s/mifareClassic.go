...
func ConnectMclassic(r *Reader) (mifare.Classic, error) {
...
func MClassic(c *Card) (mifare.Classic, error) {
...
func (mc *mifareClassic) Apdu(apdu []byte) ([]byte, error) {
	response, err := mc.ICard.Apdu(apdu)
	if err != nil {
		return nil, err
	}
...
func (mc *mifareClassic) valueop(valOp, bNr byte, value []byte) error {
	...
	response, err := mc.ICard.Apdu(aid)
	if err != nil {
		return err
	}
	if err := mifare.VerifyResponseIso7816(response); err != nil {
...
func (mc *mifareClassic) Auth(bNr, keyType int, key []byte) ([]byte, error) {
	aid := []byte{0xFF, 0x82, 0x00, 0x20, 0x06}
	aid = append(aid, key...)
	...
	if keyType == 0 {
		aid = []byte{0xFF, 0x86, 0x00, 0x00, 0x05, 0x01, 0x00, byte(bNr), 0x60, 0x20}
	} else {
		aid = []byte{0xFF, 0x86, 0x00, 0x00, 0x05, 0x01, 0x00, byte(bNr), 0x61, 0x20}
	}
